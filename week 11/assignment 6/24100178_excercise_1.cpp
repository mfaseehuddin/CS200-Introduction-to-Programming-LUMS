...
public:
    ...
    {
        os << "Hostel Number: " << reservation.hostelNumber << endl;
        os << "Room Number: " << reservation.roomNumber << endl;
        os << "Hostel Type: " << reservation.hostelType << endl;
        os << "Wait List: " << reservation.waitList << endl;
        os << "Payment: " << reservation.payment << endl;
        os << "Number of Days: " << reservation.numberOfDays << endl;
        os << "Check In Date: " << reservation.checkInDate << endl;
        return os;
    }
};
class Delegate
...
public:
    ...
    Delegate **getOccupants() { return this->occupants; }
...
public:
    ...
    virtual void queryFreeRooms(int searchOcc)
    {
        ...
        for (int fr = 0; fr < totalFreeRooms; fr++)
        {
            ...
            if (freeRooms[fr] != nullptr)
            {
                ...
                {
                    // ret all rooms with no occupants
                    if (freeRooms[fr]->getNumOccupants() == 0)
                    {
                        cout << "\t" << color
                             << name << " R-" << setfill('0') << setw(2) << freeRooms[fr]->getRoomNumber()
                             << "\033[0m"
    ...
    virtual void generateReservedRoomsReport()
    {
        cout << "\tRoom\t\tMax Occupancy" << endl;
        for (int fr = 0; fr < totalFreeRooms; fr++)
        {
            if (freeRooms[fr] != nullptr)
            {
                if (freeRooms[fr]->getNumOccupants() > 0)
                {
                    ...
                    cout << "\t" << color
                         ...
                         << "\t" << freeRooms[fr]->getNumOccupants() << "/" << freeRooms[fr]->getOccupancyLimit()
                         << endl;
                    cout << "Room Occupants:" << endl;
                    for (int i = 0; i < freeRooms[fr]->getNumOccupants(); i++)
                    {
                        cout << "Delegate " << freeRooms[fr]->getOccupants()[i]->getName() << ": \n";
                        cout << *(freeRooms[fr]->getOccupants()[i]->getReservation()) << endl;
                    }
                    cout << endl;
                }
            }
...
struct LUMSHostels
{
    ...
    void generateEmptyRoomsReport()
    {
        queryFreeRooms('B', -1);
    }

    void generateReservedRoomsReport()
    {
        cout << "Male Hostels:\n";
        for (int mHos = 0; mHos < totalMaleHostels; mHos++)
        {
            hostels[male][mHos]->generateReservedRoomsReport();
        }
        cout << "\nFemale Hostels:\n";
        for (int fHos = 0; fHos < totalFemaleHostels; fHos++)
        {
            hostels[female][fHos]->generateReservedRoomsReport();
        }
    }

    void generateIncomeReport()
    {
        // we will sum all the incomes of all the hostels based on hostel type and occupancy type
        int maleDoubleIncome = 0, femaleSingleIncome = 0, femaleDoubleIncome = 0;
        int totalIncome = 0;
        // Calculating maleDouble Income
        for (int mHos = 0; mHos < totalMaleHostels; mHos++)
        {
            for (int room = 0; room < hostels[male][mHos]->getNumberOfRooms(); room++)
            {
                for (int occupant = 0; occupant < hostels[male][mHos]->operator()(room).getNumOccupants(); occupant++)
                {
                    maleDoubleIncome += hostels[male][mHos]->operator()(room).getOccupants()[occupant]->getReservation()->getPayment();
                }
            }
        }

        // Calculating femaleSingle & femaleDouble Income
        for (int fHos = 0; fHos < totalFemaleHostels; fHos++)
        {
            for (int room = 0; room < hostels[female][fHos]->getNumberOfRooms(); room++)
            {
                for (int occupant = 0; occupant < hostels[female][fHos]->operator()(room).getNumOccupants(); occupant++)
                {
                    if (hostels[female][fHos]->operator()(room).getOccupancyLimit() == 2)
                    {
                        femaleDoubleIncome += hostels[female][fHos]->operator()(room).getOccupants()[occupant]->getReservation()->getPayment();
                    }
                    else if (hostels[female][fHos]->operator()(room).getOccupancyLimit() == 1)
                    {
                        femaleSingleIncome += hostels[female][fHos]->operator()(room).getOccupants()[occupant]->getReservation()->getPayment();
                    }
                    else
                    {
                        throw "Invalid occupancy limit";
                    }
                }
            }
        }
        ...
        cout << "Income:\n"
             << "Male Hostels Double Occupancy: Rs." << maleDoubleIncome << '\n'
             << "Female Hostels Single Occupancy: Rs." << femaleSingleIncome << '\n'
             << "Female Hostels Double Occupancy: Rs." << femaleDoubleIncome << '\n'
             << "Total Income: Rs." << maleDoubleIncome + femaleSingleIncome + femaleDoubleIncome << '\n';
    }
};
...
struct Waitlist
{
    ...
    void generateWaitListReport()
    {
        cout << "Waitlist:\n";
        for (int i = 0; i < waitListSize; i++)
        {
            cout << "Name: " << waitList[i]->getName();
            cout << "\nCNIC: " << waitList[i]->getCNIC();
            cout << "\nPhone Number: " << waitList[i]->getPhone();
            cout << "\nWaitlist Request: " << waitList[i]->getWaitListQuery();
            cout << "\n"
                 << (waitList[i]->getWaitListQuery()[0] == 'M' ? "Male Hostel " : "Female Hostel ") << (waitList[i]->getWaitListQuery()[1] == 1 ? "Single Occupancy" : "Double Occupancy") << "\n\n";
        }
    }
};
...

{
    ...
    void reserveRoom(string reservationQuery)
    {
        ...
        while (true)
        {
            ...
            if (checkInDay + reservationDays > 9)
            {
    ...
    void checkOut()
    {
        // Guard Clause->if the room is not reserved or waitlisted, then it will return
        if (!roomAllotted && !waitListed)
        {
...
void AdminMenu(LUMUNManagment &lumunManagment)
{
    int choice;
    while (true)
    {
        cout << "1. Generate Empty Rooms Report" << endl;
        cout << "2. Generate Reserved Rooms Report" << endl;
        cout << "3. Generate Waitlist Report" << endl;
        cout << "4. Generate Income Report" << endl;
        cout << "5. Back to menu" << endl;
        cout << "Enter your choice: ";
        int choices[] = {1, 2, 3, 4, 5};
        choice = userIntPut(choices, 5);
        ...
        {
        case 1:
            // Generate Empty Rooms Report
            lumunManagment.hostels->generateEmptyRoomsReport();
            break;
        case 2:
            // Generate Reserved Rooms Report
            lumunManagment.hostels->generateReservedRoomsReport();
            break;
        case 3:
            // Generate Waitlist Report
            lumunManagment.waitList->generateWaitListReport();
            break;
        case 4:
            // Generate Income Report
            lumunManagment.hostels->generateIncomeReport();
            break;
        case 5:
            // Back to menu
            return;
